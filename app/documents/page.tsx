      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-4xl font-bold text-slate-900 mb-2">الوثائق</h1>
          ...
          {!documents || documents.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-slate-600 mb-4">ابدأ بإضافة وثيقة نقل جديدة</p>
                <Link href="/documents/new">
                  <Button>
                    <Plus className="h-4 w-4 ml-2" />
                    إضافة وثيقة
                  ...
                  <Card className="hover:shadow-lg transition-shadow cursor-pointer">
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        <div>
                          <CardTitle className="text-xl mb-2">وثيقة رقم: {doc.document_number}</CardTitle>
                          <div className="flex gap-4 text-sm text-slate-600">
                            <span>الحالة: {doc.status}</span>
                            {doc.receipt_date && <span>تاريخ الاستلام: {doc.receipt_date}</span>}
                          </div>
                        </div>
                        <div className="text-left">
                          <div className="text-sm text-slate-500">
                            {doc.created_at?.toDate
                              ? new Date(doc.created_at.toDate()).toLocaleDateString("ar-SA")
                              : ""}
                          </div>
                        </div>
                      </div>
